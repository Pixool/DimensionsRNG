...
ShopItems = {
    "Glimmerwood Lantern": {"Requirements"}
}
...
def shortinize(num):
    if len(str(num)) > 12:
        return f'{str(num / 1000000000000).replace('.0', '')}T'
    if len(str(num)) > 9:
        return f'{str(num / 1000000000).replace('.0', '')}B'
    if len(str(num)) > 6:
        return f'{str(num / 1000000).replace('.0', '')}M'
    return str(num).replace('.0', '')

def converttotime(num):
    h, m, s = 0, 0, num
    m = math.floor(s / 60)
    s -= m*60
    h = math.floor(m / 60)
...
def render_frame():
    os.system("cls")
    print("Dimension RNG v0.7")
    print("-----------------------")
    if lls != False:
        print(f"Luck Bonus: {lii * 100 - 100}% left for another {converttotime(round(lls - time.time()))}")
    else:
...
while True:
    ...
    if answer == 'b':
        while True:
            def lp(name, aura):
                if name == 'Error':
                    ...
                    print(f"Rarity: 1/{shortinize(math.ceil(int(1/aura['Chance'])))}")
                    print()
                    print(aura["Desc"])
                    print()
                    input("Press enter to return to index main menu ")
            ...
            for i, (name, _) in enumerate(Auras2):
                print(f"[#{i+1}] {name} - 1/{shortinize(math.ceil(1/Auras[name]['Chance']))}")
            if e == True:
        ...
        while True:
            ...
            if number == 0:
                pas = True
                number = input("How many times would you like to roll? ")
                if number.isdigit():
                    if int(number) != 0:
                        pas = False
                while pas:
                    render_frame()
                    print("Invalid input, try agian, type 'q' to quite")
                    number = input("How many times would you like to roll? ")
                    if number == 'q':
                        break
                    if number.isdigit():
            ...
            for _ in range(int(number)):
                ...
                for seg in segments:
                    if seg[0] <= x <= seg[1]:
                        if tingy == False:
                            print('Rolled a lucky number!!! ~ 1/2B')
                            print(f"Gained boost 5000{'%'} luck boost for 10 minutes!")
                            lls = int(time.time() + 600)
                            lii = 5001
                        else:
                            print(f"{seg[2]} 1/{shortinize(int(math.ceil(1/Auras[seg[2]]['Chance'])))}")
                            tingy = False
